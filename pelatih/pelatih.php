...
if ($msg == 'cancel') {
    ...
    <i class="fa-solid fa-xmark"></i> Data Pelatih gagal diperbarui..NIP sudah ada..
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  </div>';
}
?>
        ...
        <div class="container-fluid px-4">
            ...
            <div class="card">
                <div class="card-header">
                    <i class="fa-solid fa-list"></i> Data Pelatih
                    <a href="<?= $main_url ?>pelatih/add-pelatih.php" class="btn btn-sm btn-primary float-end"> <i
                            class="fa-solid fa-plus"></i> Tambah Pelatih</a>
                </div>
                <div class="card-body">
                    <table class="table table-hover" id="datatablesSimple">
                        ...
                        <tbody>
                            ...
                            while ($data = mysqli_fetch_array($queryPelatih)) {
                                ?>
                                <tr>
                                    <th scope="row">
                                        <?= $no++ ?>
                                    ...
                                    <td align="center">
                                        <a href="edit-pelatih.php?id=<?= $data['id'] ?>" class="btn btn-sm btn-warning"
                                            title="update pelatih">
                                            <i class="fa-solid fa-pen"></i></a>
                                        <button type="button" class="btn btn-sm btn-danger" id="btnHapus"
                                            title="hapus pelatih" data-id="<?= $data['id'] ?>"
                                            data-foto="<?= $data['foto'] ?>"><i class="fa-solid fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', "#btnHapus", function () {
                $('#mdlHapus').modal('show');
                let idPelatih = $(this).data('id');
                let fotoPelatih = $(this).data('foto');
                $('#btnMdlHapus').attr("href", "hapus-pelatih.php?id=" + idPelatih + "&foto=" + fotoPelatih);
            })
        })
    </script>
